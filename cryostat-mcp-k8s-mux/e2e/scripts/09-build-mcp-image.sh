...
IMAGE_NAMESPACE="cryostat-mcp"
IMAGE_NAME="cryostat-mcp-k8s-mux"
IMAGE_TAG="latest"
LOCAL_IMAGE="${IMAGE_NAME}:${IMAGE_TAG}"
REGISTRY_IMAGE="${REGISTRY_HOST}/${IMAGE_NAMESPACE}/${IMAGE_NAME}:${IMAGE_TAG}"

echo "=== Building and pushing cryostat-mcp-k8s-mux container image ==="
...
echo "cryostat-mcp-k8s-mux directory: $K8S_MULTI_MCP_DIR"
echo "Building local image: ${LOCAL_IMAGE}"
echo ""

# Build the container image using Maven (local tag)
echo "Building with Maven..."
./mvnw clean package -pl cryostat-mcp-k8s-mux -am -DskipTests \
