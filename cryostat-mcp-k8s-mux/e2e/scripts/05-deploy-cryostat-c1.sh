...
# Script: 05-deploy-cryostat-c1.sh
# Purpose: Deploy Cryostat c1 instance using Cryostat Operator CR
...
NAMESPACE="c1"
CRYOSTAT_NAME="cryostat-c1"
MANIFEST_FILE="${MANIFESTS_DIR}/cryostat-c1.yaml"
...
echo ""
echo "Waiting for Cryostat c1 to be ready..."
...
if [ $ELAPSED -ge $TIMEOUT ]; then
    echo "  ✗ Timeout waiting for Cryostat c1"
    echo ""
    echo "Current status:"
...
if oc get route -n "$NAMESPACE" "$CRYOSTAT_NAME" &> /dev/null; then
    ...
    echo "  ✓ Cryostat c1 Route: https://${ROUTE}"
else
    echo "  ✗ Warning: Cryostat c1 Route not found"
fi

echo ""
echo "✓ Cryostat c1 deployed successfully"
echo ""
