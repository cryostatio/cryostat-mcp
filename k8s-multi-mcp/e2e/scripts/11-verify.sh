...
REQUIRED_PROJECTS=("cryostat-multi-mcp" "c1" "apps1" "c2" "apps2")
...
if oc get pods -n cryostat-multi-mcp -l app.kubernetes.io/name=cryostat-k8s-multi-mcp &> /dev/null; then
    READY=$(oc get pods -n cryostat-multi-mcp -l app.kubernetes.io/name=cryostat-k8s-multi-mcp -o jsonpath='{.items[0].status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "False")
    if [[ "$READY" == "True" ]]; then
        echo "  ✓ k8s-multi-mcp is running"
    else
        echo "  ✗ k8s-multi-mcp is not ready"
        oc get pods -n cryostat-multi-mcp -l app.kubernetes.io/name=cryostat-k8s-multi-mcp
        exit 1
    fi
else
    echo "  ✗ k8s-multi-mcp not found"
    oc get all -n cryostat-multi-mcp
    exit 1
fi
...
# Check k8s-multi-mcp Route
MCP_ROUTE_NAME=$(oc get route -n cryostat-multi-mcp -o name | head -n 1 | cut -d'/' -f2)
if [ -n "$MCP_ROUTE_NAME" ]; then
    MCP_ROUTE=$(oc get route -n cryostat-multi-mcp "$MCP_ROUTE_NAME" -o jsonpath='{.spec.host}')
    ...
    echo "  ✗ k8s-multi-mcp Route not found"
fi
...
echo ""
echo "To view logs:"
echo "  oc logs -n cryostat-multi-mcp -l app.kubernetes.io/name=cryostat-k8s-multi-mcp -f"