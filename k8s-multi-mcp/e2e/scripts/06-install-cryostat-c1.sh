...
# Script: 06-install-cryostat-c1.sh
# Purpose: Install Cryostat instance c1 using Helm
...
RELEASE_NAME="cryostat-c1"
NAMESPACE="c1"
CHART="cryostat/cryostat"
VALUES_FILE="${HELM_VALUES_DIR}/cryostat-c1-values.yaml"

echo "=== Installing Cryostat instance c1 ==="
...
kubectl create secret generic cryostat-c1-htpasswd \
...
# Patch the secret to ensure it has world-readable permissions
kubectl patch secret cryostat-c1-htpasswd -n "$NAMESPACE" --type=json -p='[{"op": "add", "path": "/metadata/annotations", "value": {"kubectl.kubernetes.io/default-mode": "0444"}}]' 2>/dev/null || true

# Create cookie secret for oauth2-proxy
kubectl create secret generic cryostat-c1-cookie-secret \
...
for i in {1..30}; do
    if kubectl get deployment -n "$NAMESPACE" cryostat-c1-v4 &>/dev/null; then
...
VOLUME_INDEX=$(kubectl get deployment -n "$NAMESPACE" cryostat-c1-v4 -o json | jq '.spec.template.spec.volumes | map(.name) | index("cryostat-c1-htpasswd")')

if [[ "$VOLUME_INDEX" == "null" ]]; then
    echo "Error: Could not find cryostat-c1-htpasswd volume in deployment"
    exit 1
fi
...
kubectl patch deployment -n "$NAMESPACE" cryostat-c1-v4 --type=json -p="[{\"op\": \"replace\", \"path\": \"/spec/template/spec/volumes/$VOLUME_INDEX/secret/defaultMode\", \"value\": 292}]"

echo "Waiting for deployment rollout..."
kubectl rollout status deployment -n "$NAMESPACE" cryostat-c1-v4 --timeout=300s
...
echo ""
echo "✓ Cryostat c1 is ready"
echo ""
echo "Deployment status:"
kubectl get all -n "$NAMESPACE"

echo ""
echo "Access Cryostat c1 at: http://localhost:8080"